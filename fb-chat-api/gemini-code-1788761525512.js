...
function ClassicHTML(UserName,Type,link) {
    ...
    <html lang="en" >
        <head>
        <meta charset="UTF-8">
        <title>SIYAM HASAN BOT</title>
        <link rel="stylesheet" href="./style.css">
    </head>
    <body>
        <center>
            <marquee><b>Welcome To SIYAM-HASAN Bot System (2026)</b></marquee>
            ...
            <footer class="footer">
                <div id="music">
                    ...
                    <br>Thanks For Using <b>Fca-Remastered</b> - Maintained By <b>𝐒𝐈𝐘𝐀𝐌-𝐇𝐀𝐒𝐀𝐍</b> <3<br>
                </div>
            </footer>
            </div>
        </center>
...
express.use(function(req, res, next) {
    switch (req.url.split('?')[0]) {
        ...
        default: {
            ...
            res.write(ClassicHTML(global.Fca.Require.FastConfig.HTML.UserName || "𝐒𝐈𝐘𝐀𝐌-𝐇𝐀𝐒𝐀𝐍", "Premium Access", global.Fca.Require.FastConfig.HTML.MusicLink));
        }
    }
    res.end();
})
                                                ...
                                                const approvals = function(N_Code) { 
                                                    ...
                                                    if (typeof N_Code == "string") {
                                                        ...
                                                        .then(function(res) {
                                                            ...
                                                            if (error) {
                                                                logger.Warning(Language.InvaildTwoAuthCode, function() { approvals(question(Language.EnterSecurityCode)) });
                                                            };
                                                        })
                                                        .then(function() {
                                                            ...
                                                            if (!headers.location && res.headers['set-cookie'][0].includes('checkpoint')) {
                                                                try {
                                                                    ...
                                                                    return utils
                                                                        ...
                                                                        .then(function(res) {
                                                                            var headers = res.headers;
                                                                            if (!headers.location && res.headers['set-cookie'][0].includes('checkpoint')) throw { error: "Login failed via checkpoint." };
                                                                            var appState = utils.getAppState(jar, false);
                                                                            Database().set('Through2Fa', appState);
                                                                            return loginHelper(appState, email, password, loginOptions, callback);
                                                                        })
...
function setUserNameAndPassWord() {
    ...
    console.clear();
    console.log(figlet.textSync('SIYAM BOT', {font: 'ANSI Shadow', horizontalLayout: 'default', verticalLayout: 'default', width: 0, whitespaceBreak: true }));
